...
const userModule = require("../../modules/client/user_module");
...
router.get("/", auth, (req, res) => {
  userModule
    ...
    .catch((error) => {
      logger.error(`GET /clients/users error : ${error.message}`);
...
router.get("/:name", auth, (req, res) => {
  userModule
    ...
    .catch((error) => {
      logger.error(`GET /clients/users/:name error : ${error.message}`);
      res.status(500).send(error.message);
    });
});
...
router.post("/", auth, (req, res) => {
  userModule
    ...
    .catch((error) => {
      logger.error(`POST /clients/users error : ${error.message}`);
      res.status(500).send(error.message);
    });
});
...
router.put("/pass/:id", auth, (req, res) => {
  userModule
    ...
    .catch((error) => {
      logger.error(`PUT /clients/users error : ${error.message}`);
      res.status(500).send(error.message);
    });
});
